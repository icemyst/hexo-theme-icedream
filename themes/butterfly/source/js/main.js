...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const sidebarFn = {
    open: () => {
      btf.sidebarPaddingR();
      btf.changeThemeMetaColor("#607d8b");
  ...
  const scrollDownInIndex = () => {
    ...
    $scrollDownEle &&
      $scrollDownEle.addEventListener("click", function () {
        if ($bbTimeList) {
          btf.scrollToDest($bbTimeList.offsetTop, 300);
        } else {
  ...
  const scrollFnToDo = function () {
    ...
    if (isToc) {
      ...
      $cardToc.addEventListener("click", e => {
        ...
        btf.scrollToDest(
          btf.getEleTop(document.getElementById(decodeURI($target.getAttribute("href")).replace("#", ""))) - 60,
          300
        );
  ...
  const tabsFn = {
    ...
    backToTop: () => {
      document.querySelectorAll("#article-container .tabs .tab-to-top").forEach(function (item) {
        item.addEventListener("click", function () {
          btf.scrollToDest(btf.getEleTop(btf.getParents(this, ".tabs")) - 60, 300);
        });
      });
    },
  };
  ...
  const relativeDate = function (selector, simple = false) {
    selector.forEach(item => {
      ...
      if (simple) {
        $this.innerText = btf.diffDate(timeVal, false, simple);
      } else {
  ...
  const coverColor = async () => {
    ...
    if (GLOBAL_CONFIG.mainTone) {
      if (GLOBAL_CONFIG_SITE.postMainColor) {
        ...
        root.style.setProperty("--icedream-bar-background", value);
        btf.initThemeColor();

        if (GLOBAL_CONFIG.mainTone.cover_change) {
          document.documentElement.style.setProperty("--icedream-main", value);
          document.documentElement.style.setProperty(
            "--icedream-theme-op",
            getComputedStyle(document.documentElement).getPropertyValue("--icedream-main") + "23"
          );
        ...
        const fallbackValue = "var(--icedream-theme)";
        let fetchPath = "";
            ...
            if (GLOBAL_CONFIG.mainTone.mode == "both") {
              ...
              try {
                ...
                if (response.ok && response.headers.get("content-type")?.includes("application/json")) {
                  ...
                  root.style.setProperty("--icedream-bar-background", fallbackValue);
                  btf.initThemeColor();
                  document.documentElement.style.setProperty("--icedream-main", fallbackValue);
                }
              } catch {
                root.style.setProperty("--icedream-bar-background", fallbackValue);
                btf.initThemeColor();
