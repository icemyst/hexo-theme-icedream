const btf = {
  ...
  snackbarShow: (text, showActionFunction = false, duration = 2000, actionText = false) => {
    ...
    root.style.setProperty("--icedream-snackbar-time", duration + "ms");
  ...
  initThemeColor: function () {
    let themeColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--icedream-bar-background")
      .trim()
      .replace('"', "")
      .replace('"', "");
    ...
    if (currentTop > 26) {
      if (btf.is_Post()) {
        themeColor = getComputedStyle(document.documentElement)
          .getPropertyValue("--icedream-meta-theme-post-color")
          .trim()
          .replace('"', "")
          .replace('"', "");
      }
  ...
  switchDarkMode: () => {
    ...
    const root = document.querySelector(":root");
    root.style.setProperty("--icedream-bar-background", "var(--icedream-meta-theme-color)");
    btf.initThemeColor();

    // 要改回来默认主色
    document.documentElement.style.setProperty(
      "--icedream-main",
      getComputedStyle(document.documentElement).getPropertyValue("--icedream-theme")
    );
    document.documentElement.style.setProperty(
      "--icedream-theme-op",
      getComputedStyle(document.documentElement).getPropertyValue("--icedream-main") + "23"
    );
    document.documentElement.style.setProperty(
      "--icedream-theme-op-deep",
      getComputedStyle(document.documentElement).getPropertyValue("--icedream-main") + "dd"
    );
  },
  //是否是文章页
  is_Post: function () {
  ...
  catalogActive: function () {
    ...
    if ($list) {
      ...
      btf.scrollByMouseWheel($list, $catalog);
    }
  },
  // Page Tag 激活
  tagsPageActive: function () {
    ...
    if ($list) {
      ...
      btf.scrollByMouseWheel($list, $tagPageTags);
    }
  },
  // 修改时间显示"最近"
  ...
  changeTimeInEssay: function () {
    document.querySelector("#bber") &&
      document.querySelectorAll("#bber time").forEach(function (e) {
        ...
        (t.innerText = btf.diffDate(datetime, true)), (t.style.display = "inline");
      });
  },
  // 修改相册集中的时间
  ...
  getCustomPlayList: function () {
    ...
    if (urlParams.get("id") && urlParams.get("server")) {
      ...
      anMusicPageMeting.innerHTML = `<meting-js id="${id}" server=${server} type="playlist" type="playlist" mutex="true" preload="auto" theme="var(--icedream-main)" order="list" list-max-height="calc(100vh - 169px)!important"></meting-js>`;
    } else {
      anMusicPageMeting.innerHTML = `<meting-js id="${userId}" server="${userServer}" type="playlist" mutex="true" preload="auto" theme="var(--icedream-main)" order="list" list-max-height="calc(100vh - 169px)!important"></meting-js>`;
    }
    btf.changeMusicBg(false);
  },
  //隐藏今日推荐
